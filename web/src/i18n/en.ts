...
export const en: Translations = {
  ...
  status: {
    ...
    updateHermesConfirmMessage:
      "This runs forx update and restarts the gateway when it finishes. Active sessions keep their prompt cache until then.",
    updateHermesConfirmNow: "Update now",
    updateHermesConfirmTitle: "Update ForX?",
    updatingHermes: "Updating ForX…",
  ...
  pluginsPage: {
    ...
    headline:
      "Discover, install, enable, and update ForX plugins (`forx plugins` parity).",
    ...
    removeConfirm: "Remove this plugin from ~/.forx/plugins/?",
    removeHint: "Only user-installed plugins under ~/.forx/plugins can be removed.",
    ...
    catalogEmptyDocsLink: "Learn about ForX plugins",
  ...
  skills: {
    ...
    filters: "Filters",
    noSkills: "No skills installed yet. Browse the skills hub or create one here; from a terminal you can also run `forx skills search <topic>`.",
  ...
  env: {
    ...
    customHint: "Arbitrary environment variables stored in your .env that ForX doesn't recognise. Use these to inject env vars for skills, MCP servers, or your own tooling.",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Collectible ForX badges earned from real session history. Known unfinished achievements are shown as Discovered; Secret achievements stay hidden until the first matching behavior appears.",
      scan_subtitle:
        "Scanning ForX session history. First scan can take 5–10 seconds on large histories.",
    },
    actions: {
      rescan: "Rescan",
    },
    stats: {
      ...
      latest: "Latest",
      latest_hint_empty: "run ForX more",
      none_yet: "None yet",
    },
    state: {
      unlocked: "Unlocked",
    ...
    guide: {
      ...
      secret_body:
        "Secrets hide their exact trigger. Once ForX sees a related signal, the card becomes Discovered and shows its requirement.",
      scan_status_header: "Scan status",
      scan_status_body:
        "ForX is scanning local history once, then cards will appear automatically. Nothing is stuck if this takes a few seconds.",
